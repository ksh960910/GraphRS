...
def generate_sampled_graph(directory, iteration):
    try:
        zeta = np.load(directory + 'zeta/zeta.npy')
        sampled_graph = directory + 'sampled_graph/sampled_graph_epoch' + str(iteration) + '_epsilon' + str(0.01)
        ...
        generate_graph(directory).generate_graph(zeta=zeta, epsilon=0.01, iteration=iteration)
        # sampled_graph = self.path + '/sampled_graph/sampled_graph_' + str(iteration+1)
...
def read_cf_gowalla_train(directory, iteration):
    sampled_graph = generate_sampled_graph(directory, iteration)
    print(sampled_graph)
...
def load_data(model_args, iteration):
    global args, dataset
    args = model_args
    ...
    if sample_method==1:
        train_cf = read_cf(directory,iteration)
