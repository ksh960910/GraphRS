...
class XmixSimGCL(nn.Module):
    ...
    def get_user_sim_score(self, support_user_view, neg_user_view, u_idx):
        support_user_view, neg_user_view = F.normalize(support_user_view, dim=1), F.normalize(neg_user_view, dim=1)
        score = torch.matmul(support_user_view[u_idx], neg_user_view[u_idx].t())
        return score
    
    def get_item_sim_score(self, support_item_view, neg_item_view, i_idx):
        support_item_view, neg_item_view = F.normalize(support_item_view, dim=1), F.normalize(neg_item_view, dim=1)
        score = torch.matmul(support_item_view[i_idx], neg_item_view[i_idx].t())
        return score
    ...
    def fn_detection(self, score, k):
        ...
        '''threshold strat'''
        t_mask = torch.where(score>=self.threshold, 1, 0).cuda()
    ...
    def cal_cl_loss(self, idx, user_view1, user_view2, item_view1, item_view2, neg_candidate_users, neg_candidate_items):
        ...
        user_scores = self.get_user_sim_score(cl_user_view0, neg_user_view, u_idx)
        item_scores = self.get_item_sim_score(cl_item_view0, neg_item_view, i_idx)
        ...
        user_cl_loss = self.InfoNCE(user_view1[u_idx], user_view2[u_idx], neg_user_view[u_idx], user_fn_mask, self.temp)
        item_cl_loss = self.InfoNCE(item_view1[i_idx], item_view2[i_idx], neg_item_view[i_idx], item_fn_mask ,self.temp)
        return user_cl_loss + item_cl_loss

    def InfoNCE(self, view1, view2, neg_view, mask, temperature, b_cos=True):
        if b_cos:
            view1, view2, neg_view = F.normalize(view1, dim=1), F.normalize(view2, dim=1), F.normalize(neg_view, dim=1)

        LARGE_NUM = 1e9
        # python main.py --dataset gowalla --gnn xmgcl --dim 64 --lr 0.001 --batch_size 2048 --gpu_id 0 --context_hops 3 --pool mean --ns mixgcf --n_negs 32 --K 1 --fnk 1
