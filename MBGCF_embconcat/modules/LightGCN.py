...
class GraphConv(nn.Module):
    ...
    """
    def __init__(self, n_hops, n_users, interact_mat, interact_mat_obs,
                 edge_dropout_rate=0.5, mess_dropout_rate=0.1):
        super(GraphConv, self).__init__()

        self.interact_mat = interact_mat
        self.interact_mat_obs = interact_mat_obs
    ...
    '''
    추가해야하는 것 : observed graph와 sample graph의 embedding을 concat하는것
    '''
    def forward(self, user_embed, item_embed,
        ...
        for hop in range(self.n_hops):
            ...
            interact_mat_obs = self._sparse_dropout(self.interact_mat_obs,
                                                    self.edge_dropout_rate) if edge_dropout \
                                                                            else self.interact_mat_obs

            agg_embed_sample = torch.sparse.mm(interact_mat, agg_embed)
            agg_embed_obs = torch.sparse.mm(interact_mat_obs, agg_embed)
            if mess_dropout:
                agg_embed_sample = self.dropout(agg_embed_sample)
                agg_embed_obs = self.dropout(agg_embed_obs)
            # agg_embed = F.normalize(agg_embed)
            agg_embed_concat = torch.cat((agg_embed_sample, agg_embed_obs))
            embs.append(agg_embed_concat)
...
class LightGCN(nn.Module):
    ...
    def _init_model(self):
        return GraphConv(n_hops=self.context_hops,
                         ...
                         interact_mat_obs = self.sparse_norm_adj_obs,
    ...
    def forward(self, batch=None):
        ...
        # return user_gcn_emb[user], item_gcn_emb[pos_item], neg_gcn_embs
