...
def get_performance(user_pos_test, r, auc, Ks, deg_item, k_max_items):
    ...
    for K in Ks:
        ...
        novelty.append(novelty_at_k(K, n_test_users, deg_item, k_max_items))
        hit_ratio.append(hit_at_k(r, K))

    return {'recall': np.array(recall), 'precision': np.array(precision), 'novelty' : np.array(novelty),
...
def test(model, user_dict, n_params, deg_item, mode='test'):
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        for re in batch_result:
            ...
            result['novelty'] += re['novelty']/n_test_users
