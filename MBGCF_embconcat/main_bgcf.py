...
from utils.data_loader import load_data, load_observed_data
...
if __name__ == '__main__':
    ...
    """build dataset"""
    train_cf, user_dict, n_params, norm_mat, deg_item = load_data(args)
    train_cf_size = len(train_cf)
    train_cf = torch.LongTensor(np.array([[cf[0], cf[1]] for cf in train_cf], np.int32))

    '''여기에 sampled graph에 대한 train_cf 말고 observed graph
       에 대한 train_cf가 추가돼야함''' 
    train_cf_observed, user_dict_observed, n_params_observed, norm_mat_observed, deg_item_obs = load_observed_data(args)
    train_cf_observed_size = len(train_cf_observed)
    train_cf_observed = torch.LongTensor(np.array([[cf[0], cf[1]] for cf in train_cf_observed], np.int32))
    ...
    if args.gnn == 'lightgcn':
        model = LightGCN(n_params, args, norm_mat, norm_mat_observed).to(device)
    else:
    ...
    for epoch in range(args.epoch):
        ...
        ''' 여기가 추가한 부분'''
        train_cf_observed_ = train_cf_observed
        index_observed = np.arange(len(train_cf_observed_))
        np.random.shuffle(index_observed)
        train_cf_observed_ = train_cf_observed[index_observed].to(device)
        ...
        while s + args.batch_size <= len(train_cf):
            ...
            # batch_obs = get_feed_dict(train_cf_observed_,
            #                         user_dict_observed['train_user_set'],
            #                         s, s + args.batch_size,
            #                         n_negs)

            batch_loss, _, _ = model(batch)
            # user_gcn_emb, pos_gcn_emb, neg_gcn_emb = model(batch)
            # batch_loss, _, _ = model.create_bpr_loss(user_gcn_emb, pos_gcn_emb, neg_gcn_emb)
        ...
        if epoch % 5 == 0:
            ...
            model.eval()
            test_s_t = time()
            test_ret = test(model, user_dict, n_params, deg_item_obs, mode='test')
            test_e_t = time()
            train_res.add_row(
            ...
            if user_dict['valid_user_set'] is None:
                ...
                test_s_t = time()
                valid_ret = test(model, user_dict, n_params, deg_item_obs, mode='valid')
                test_e_t = time()
                train_res.add_row(
