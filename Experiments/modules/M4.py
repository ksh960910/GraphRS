...
class M4(nn.Module):
    ...
    def forward(self, batch=None):
        ...
        # s_user_view1, s_item_view1 = self.create_support_set()
    ...
    def cal_cl_loss(self, idx):
        ...
        s_user_view2, _ = self.generate()
        # user_fn_idx : [batch_size, top-k]
        user_scores_1 = self.get_user_sim_score(s_user_view1, user_view2, u_idx)
        user_scores_2 = self.get_user_sim_score(s_user_view2, user_view2, u_idx)
        user_scores, _ = torch.stack([user_scores_1, user_scores_2]).max(dim=0)
        # max method
        # mean method
    ...
    # def cal_cl_loss(self, idx, rec_user_emb, rec_item_emb, layer_user_emb, layer_item_emb):
    #     u_idx = torch.unique(torch.LongTensor(idx[0].cpu()).type(torch.long)).to(self.device)
    #     i_idx = torch.unique(torch.LongTensor(idx[1].cpu()).type(torch.long)).to(self.device)
    #     user_view1, item_view1 = self.generate()
    #     user_view2, item_view2 = self.generate()
    ...
    #     item_scores = self.get_item_sim_score(layer_item_emb, item_view2, i_idx)

    #     # 2
    #     # user_scores = self.get_user_sim_score(rec_user_emb, user_view2, u_idx)
    #     # item_scores = self.get_item_sim_score(rec_item_emb, item_view2, i_idx)
