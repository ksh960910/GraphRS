...
class M3(nn.Module):
    def __init__(self, data_config, args_config, adj_mat, neighbor_dict, deg_dict):
    ...
    def forward(self, batch=None):
        ...
        # cl_loss = self.cl_rate * self.cal_cl_loss([user, pos_item], rec_user_emb, rec_item_emb, layer_user_emb, layer_item_emb)
    ...
    def cal_cl_loss(self, idx):
        ...
        s_user_view1, s_item_view2 = self.generate()
        
        # user_fn_idx : [batch_size, top-k]
        '''주석처리 부분'''
        # user_scores = self.get_user_sim_score(s_user_view1, user_view2, u_idx)

        # m_mask = self.degree_mask(self.neighbor_dict, self.deg_dict, u_idx)
        # # print('m mask shape : ', m_mask.shape)
    
        # user_fn_mask = self.fn_detection(m_mask, user_scores, self.fnk)
        ...
        '''주석처리 부분'''
        user_cl_loss =self.InfoNCE_item(user_view1[u_idx], user_view2[u_idx], self.temp)
    ...
    #     return s_user_view1, s_item_view1
    ...
    # def degree_mask(self, neighbor_dict, deg_dict, u_idx):
    #     u_idx = u_idx.tolist()
    #     t = list()
    #     for i in u_idx:
    #         r = list()
    #         for j in u_idx:
    #             intersection = float(len(list(set(neighbor_dict[i]).intersection(neighbor_dict[j]))))
    #             deg_ratio = intersection / deg_dict[i]
    #             r.append(deg_ratio)
    #         t.append(r)
    #     mask = torch.Tensor(t)
    #     return mask
    ...
    def degree_mask(self, neighbor_dict, deg_dict, u_idx):
        u_idx = u_idx.tolist()
        t = np.zeros((len(u_idx), len(u_idx)))
        for i in range(len(u_idx)):
            for j in range(len(u_idx)):
                if i==j:
                    continue
                intersection = len(set(neighbor_dict[i]).intersection(neighbor_dict[j]))
                deg_ratio = intersection / deg_dict[i]
                t[i, j] = deg_ratio
        mask = torch.Tensor(t)
        return mask
    ...
    # def degree_mask(self, neighbor_dict, deg_dict, u_idx):
    #     u_idx = u_idx.tolist()
    #     t = torch.zeros((len(u_idx), len(u_idx)))
    #     for i in range(len(u_idx)):
    #         for j in range(i + 1, len(u_idx)):
    #             intersection = set(neighbor_dict[i]).intersection(neighbor_dict[j])
    #             t[i, j] = len(intersection) / deg_dict[i]
    #             t[j, i] = len(intersection) / deg_dict[j]
    #     mask = t
    #     return mask

    def fn_detection(self, m_mask, score, k):
        # FN이라고 판단한 node의 index를 return하는 함수
        '''top-k strategy
           정렬했을 때 상위 k개에 해당하는 index 위치에만 binary masking'''
        m_mask = torch.where(m_mask>=0.9, 1, 0).cuda()
        ...
        # '''threshold strategy'''
        # t_mask = torch.where(score>=0.8, 1, 0).cuda()
