...
if __name__ == '__main__':
    ...
    """build dataset"""
    # train_cf, user_dict, n_params, norm_mat, neighbor_dict, deg_dict = load_data(args)
    ...
    """define model"""
    from modules.M4 import M4
    from modules.M3 import M3
    ...
    if args.gnn == 'lightgcn':
        ...
        print('Method 2 GCL model setup')
        model = M2(n_params, args, norm_mat).to(device)
    elif args.gnn == 'm3':
        print('Method 3 GCL model setup')
        print('FN detection시 2-hop neighbor에 대한 adjacency matrix 활용')
        model = M3(n_params, args, norm_mat, neighbor_dict, deg_dict).to(device)
    elif args.gnn == 'm4':
        print('Method 4 GCL model setup')
        print('Support view 2개의 max값')
        model = M4(n_params, args, norm_mat).to(device)
    else:
