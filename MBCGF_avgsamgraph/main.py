...
if __name__ == '__main__':
    ...
    """build dataset"""
    '''이부분 수정'''
    train_cf_0, user_dict, n_params, norm_mat = load_data(args,0)
    train_cf_1, user_dict, n_params, norm_mat_1 = load_data(args,1)
    train_cf_2, user_dict, n_params, norm_mat_2 = load_data(args,2)
    train_cf_size = len(train_cf_0)
    train_cf_0 = torch.LongTensor(np.array([[cf[0], cf[1]] for cf in train_cf_0], np.int32))
    ...
    train_cf = torch.stack((train_cf_0, train_cf_1), dim=0)
    train_cf = torch.stack((train_cf, train_cf_2), dim=0)
    ...
    for epoch in range(args.epoch):
        ...
        train_cf_ = train_cf[0,:,:]
        index = np.arange(len(train_cf_))
        np.random.shuffle(index)
        train_cf_ = train_cf_[index].to(device)

        train_cf_1 = train_cf[1,:,:]
        index = np.arange(len(train_cf_1))
        np.random.shuffle(index)
        train_cf_1 = train_cf_1[index].to(device)

        train_cf_2 = train_cf[2,:,:]
        index = np.arange(len(train_cf_2))
        np.random.shuffle(index)
        train_cf_2 = train_cf_2[index].to(device)
