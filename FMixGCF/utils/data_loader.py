...
def generate_sampled_graph(directory, epoch):
    try:
        zeta = np.load(directory + 'zeta/zeta.npy')
        sampled_graph = directory + 'sampled_graph/sampled_graph_epoch' + str(epoch) + '_epsilon' + str(0.01)
        ...
        generate_graph(directory).generate_graph(zeta=zeta, epsilon=0.01, iteration=epoch)
        # sampled_graph = self.path + '/sampled_graph/sampled_graph_' + str(iteration+1)
...
def read_cf_gowalla_train(directory, epoch):
    sampled_graph = generate_sampled_graph(directory, epoch)
    print(sampled_graph)
...
def read_cf_yelp2018_train(directory, epoch):
    sampled_graph = generate_sampled_graph(directory, epoch)
    print(sampled_graph)
...
def statistics(train_data, valid_data, test_data):
    ...
    return train_user_set, test_user_set, valid_user_set
...
def load_data(model_args, sample_method, epoch=None):
    global args, dataset
    args = model_args
    ...
    if sample_method==1:
        train_cf, all_pos_ids = read_cf(directory, epoch)
    ...
    train_user_set, test_user_set, valid_user_set = statistics(train_cf, valid_cf, test_cf)
