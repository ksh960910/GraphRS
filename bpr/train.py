...
def main(args):
    ...
    for u, i, j in loader:
        ...
        if idx % args.print_every == (args.print_every - 1):
            print('idx: ', idx, 'loss: %.4f' % smooth_loss)
        if idx % args.eval_every == (args.eval_every - 1):
            plist, rlist = precision_and_recall_k(model.W.detach(),
                                                    ...
                                                    test_user_list,
                                                    klist=[5, 10, 20])
            print('idx: ', 'P@5: %.4f, P@10: %.4f P@20: %.4f, R@5: %.4f, R@10: %.4f, R@20: %.4f' % (plist[0], plist[1], plist[2], rlist[0], rlist[1], rlist[2]))
            writer.add_scalars('eval', {'P@5': plist[0],
                                                    'P@10': plist[1],
                                                    'P@20': plist[2]}, idx)
            writer.add_scalars('eval', {'R@5': rlist[0],
                                                'R@10': rlist[1],
                                                'R@20': rlist[2]}, idx)
