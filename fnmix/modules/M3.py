...
class M3(nn.Module):
    ...
    def cal_cl_loss(self, idx, layer_user_emb, neg_candidate_users):
        ...
        # only user masking
        user_cl_loss = self.InfoNCE(user_view1[u_idx], user_view2[u_idx], neg_user_view[u_idx], user_fn_mask, hns_fn_mask, self.temp)
        ...
        # hns_fn_mask = self.fn_detection(hns_scores,  self.fnk)
        # # only user masking
        # user_cl_loss = self.InfoNCE(user_view1[u_idx], user_view2[u_idx], neg_user_view[u_idx], hns_fn_mask, self.temp)
        # item_cl_loss = self.InfoNCE_item(item_view1[i_idx], item_view2[i_idx], self.temp)
    ...
    # FN elimination
    def InfoNCE(self, view1, view2, neg_view, mask, hns_mask, temperature, b_cos=True):
        if b_cos:
        ...
        hns_score = torch.exp((hns_score - hns_mask*LARGE_NUM) / temperature).sum(dim=1)
        cl_loss = -torch.log(pos_score / (hns_score + ttl_score+10e-6))

        return torch.mean(cl_loss)

    # hns에만 FN masking
    # def InfoNCE(self, view1, view2, neg_view, hns_mask, temperature, b_cos=True):
    #     if b_cos:
    #         view1, view2, neg_view = F.normalize(view1, dim=1), F.normalize(view2, dim=1), F.normalize(neg_view, dim=1)
    #     LARGE_NUM = 1e9
    #     '''original cl loss'''
