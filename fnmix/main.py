...
if __name__ == '__main__':
    ...
    if args.gnn == 'lightgcn':
        ...
        print('Mixing한 negative들을 분모에 negative pair에 추가 + FN elimination')
        model = M3(n_params, args, norm_mat).to(device)
    elif args.gnn == 'm4':
        print('Method 4 GCL model setup')
        print('Mixing한 negative들을 분모에 negative pair에 추가 (No FN elimination)')
    ...
    for epoch in range(args.epoch):
        ...
        while s + args.batch_size <= len(train_cf):
            ...
            batch_loss = model(neg_candidate_users, batch)
