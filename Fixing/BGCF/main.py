    ...
    for epoch in range(args.epoch): # args.epoch
        ...
        for itr in range(n_batch):
            ...
            # for i in range(args.sample_num):
                # sampled_graph = sampled_graph_to_matrix(path = args.path+args.dataset, iteration = i+1, batch_size = args.batch_size)
                # adj_matrix = sampled_graph.get_adj_mat()
            ...
            u_g_embeddings, pos_i_g_embeddings, neg_i_g_embeddings = model(obs_users,
                                                                        obs_pos_items,
                                                                        obs_neg_items,
                                                                        obs_adj_matrix,
                                                                        test_flag = False) # sample_num 1 이상으로 할 땐 i+1로 변경
        ...
        ret = test(model, users_to_test, obs_neg_items, test_adj_mat)
        ...
        print('Test : ', perf_str) 
        with open('training_log/'+ args.dataset +'.txt', 'a') as f:
            f.write(perf_str)
            f.write('\n')
