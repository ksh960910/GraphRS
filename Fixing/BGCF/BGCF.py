...
class BGCFLayer(nn.Module):
    ...
    def forward(self, 
        ...
        # if test_flag==False:
        #     final_u_g_embeddings = torch.zeros((self.batch_size, 192)).to(self.device)
        #     final_pos_i_g_embeddings = torch.zeros((self.batch_size, 192)).to(self.device)
        #     final_neg_i_g_embeddings = torch.zeros((self.batch_size, 192)).to(self.device)
        # else:
        #     # data = Data(path = self.path+self.dataset, batch_size = self.batch_size)
        #     # users_to_test = list(data.test_set.keys())
        #     # user_batch = len(users_to_test) // self.batch_size + 1
        #     # item_batch = range(data.n_items)
        #     # final_u_g_embeddings = torch.zeros((user_batch, 192)).to(self.device)
        #     # final_pos_i_g_embeddings= torch.zeros((len(item_batch), 192)).to(self.device)
        #     final_u_g_embeddings = torch.zeros((len(obs_users), 192)).to(self.device)
        #     final_pos_i_g_embeddings = torch.zeros((obs_pos_items, 192)).to(self.device)
        #     final_neg_i_g_embeddings = torch.zeros((obs_neg_items, 192)).to(self.device)
        
        ###################################### sample graph 여러개 해서 평균내기#################################################

        # for i in range(1, self.sample_num+1):
        #     sampled_graph = sampled_graph_to_matrix(path = self.path+self.dataset, iteration = i, batch_size=self.batch_size)
        #     # print(time() - tz)
        #     adj_matrix = sampled_graph.get_adj_mat()
        #     if test_flag==False:
        #         users, pos_items, neg_items = sampled_graph.sample(obs_users)
        #     else:
        #         users, pos_items, neg_items = obs_users, obs_pos_items, obs_neg_items

        #     sample_user_n_j, sample_item_n_j = self.get_count_neighbor(adj_matrix, iteration = i)
        #     obs_user_n_j, obs_item_n_j = self.get_count_neighbor(self.obs_adj_matrix, 0)
            
        #     adj_matrix = self._convert_sp_mat_to_sp_tensor(adj_matrix)
        #     obs_adj_matrix = self._convert_sp_mat_to_sp_tensor(self.obs_adj_matrix)
        ...
        #     if i==1:
        #         final_u_g_embeddings = (h_tilde_user / self.sample_num)
        #         final_pos_i_g_embeddings = (h_tilde_pos_item / self.sample_num)
        #         final_neg_i_g_embeddings = (h_tilde_neg_item / self.sample_num)
        #     else:
        #         final_u_g_embeddings += (h_tilde_user / self.sample_num)
        #         final_pos_i_g_embeddings += (h_tilde_pos_item / self.sample_num)
        #         final_neg_i_g_embeddings += (h_tilde_neg_item / self.sample_num)

        # return final_u_g_embeddings, final_pos_i_g_embeddings, final_neg_i_g_embeddings
